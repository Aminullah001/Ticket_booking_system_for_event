...
// mongoose.connect("mongodb://localhost:27017/eventDB"); //! Setting up the connection to mongoDB URL in .env file ---
try {
...
app.post("/createEvent", async (req, res) => {
   try {
      ...
      // Check if the event already exists by some unique identifier (e.g., title)
      const existingEvent = await Event.findOne({ title: eventData.title });

      if (existingEvent) {
         // Event already exists, return an error
